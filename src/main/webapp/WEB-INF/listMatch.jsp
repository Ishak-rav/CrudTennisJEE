...
<div style="    padding: 1.5rem;    margin-right: 0;    margin-left: 0;    border-width: .2rem;">
	<a class="btn btn-primary" href="ajoutermatch" role="button">Ajouter un match</a>
</div>
<table class="table">
  <thead>
    <tr>
  ...
  <tbody>
  	<c:forEach var="match" items = "${matchs}">
    	<tr>
      		...
      		<td><c:out value = "${match.tournoi}" /></td>
      		<td><c:out value = "${match.type_epreuve}" /></td>
      		<td><c:out value = "${match.annee_epreuve}" /></td>
      		<td><c:out value = "${match.nom_vainqueur}" /></td>
      		<td><c:out value = "${match.nom_finaliste}" /></td>
	  		<td>
	  		<c:if test="${admin == true}">
	  		  <a role="button" class="btn btn-outline-secondary" href="/AppJoueur/modifiermatch?id=${match.id}">Modifier</a>
	  		  <a role="button" class="btn btn-outline-danger" href="/AppJoueur/supprimermatch?id=${match.id}">Supprimer</a>
	  		</c:if>
	  		</td>
    	</tr>
    </c:forEach>
